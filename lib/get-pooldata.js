...
export function convertJsonDataToChartEntries(jsondata,type,source) {
    ...
    jsondata.forEach(entry => {
        let newentry={}
        //console.log("entry brut",entry)
        newentry.time = toTimestamp(entry.date);
        ...
        if (entry.blockNumber !== undefined) {
            lastBlockData[type][source] = entry.blockNumber > lastBlockData[type][source] ? entry.blockNumber.toString() : lastBlockData[type][source];
        }
...
export async function getOnchainPoolData(source,range,token0,token1,dec,fromBlock=null,toBlock=null,nbBlock=30,papi=false) {
    ...
    lastBlockData["onchain"][source] = current_block_number > lastBlockData["onchain"][source] ? current_block_number : lastBlockData["onchain"][source];
    ...
    while (fromBlockHash.toString() !== currentBlockHash.toString()) { // parcours de tous les blocks depuis toBlock jusqu'a remonter à fromBlock
        ...
        entry.poolId = pair;
        entry.timestamp = block_timestamp;
