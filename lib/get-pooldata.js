 ...
 */
 export async function getOnchainPoolData(source,range,pair,fromBlock=null,toBlock=null,nbBlock=30,step=0,papi=false) {
    ...
    for (let current_block_number of blockNumberArray) {
        let promise = getOneBlockData(current_block_number,api,pair,source,data).then(()=>{
            ++nbBlockFetch;
...
async function getOneBlockData(current_block_number,api,pair,source,data) {
        ...
        if (source === "parallel") {
            const currency0 = pair.rev ? pair.token1id : pair.token0id //"101" //DOT
            const currency1 = pair.rev ? pair.token0id : pair.token1id //"200060013" //cDOT
        ...
        //console.log("ONCHAIN ONE ENTRY",entry)
