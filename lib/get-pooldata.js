...
async function subqueryRequest(source,range,token0,token1,dec,limit=100,offset=0) {
...
export async function getSubqueryPoolData(source,range,token0,token1,dec,limit=100,offset=0) {
    ...
    while (length == limitGet && full_counter < limit) {
        data = await subqueryRequest(source,range,token0,token1,dec,limitGet,offsetGet)
        offsetGet+=limitGet;
        length = data.length
        let prev = undefined;
 ...
 */
 export async function getOnchainPoolData(source,range,token0,token1,dec,fromBlock=null,toBlock=null,nbBlock=30,papi=false) {
    ...
    let blockNumberArray=Array.from({length:(toBlock - fromBlock)},(v,k)=>k+fromBlock)
    //console.log("BlocksArray",blockNumberArray)
    let arrayPromises = [];
    let nbBlockFetch = 0
    for (let current_block_number = fromBlock; current_block_number <= toBlock; current_block_number++) {
        let promise = getOneBlockData(current_block_number,api,token0,token1,source,data).then(()=>{
            ++nbBlockFetch;
            //(blockNumberArray.length,nbBlockFetch)
        })
...
async function getOneBlockData(current_block_number,api,token0,token1,source,data) {
        ...
        let pool = api_at.query.dex.liquidityPool(pair.acalapool) // Acala only pour le moment
        //liquidity = null;
        let liquidity
        await pool.then((data)=>{liquidity=data});
        let liquidity0 = liquidity[0].toString();
        let liquidity1 = liquidity[1].toString();
        let ratio_pair = liquidity0 / liquidity1;
        let ratio_pair_rev = liquidity1 / liquidity0;
