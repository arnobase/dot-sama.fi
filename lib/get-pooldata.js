 ...
 * @param api {ApiPromise} The Api to get the data, idf specified, override the "source" param
 */
 export async function getOnchainPoolDataMultirequest(source,range,token0,token1,dec,fromBlock=null,toBlock=null,nbBlock=30,papi=false) {
    ...
    for (let current_block_number = fromBlock; current_block_number < toBlock; current_block_number++) {
        let promise = getOneBlockData(current_block_number,api,token0,token1,source,data).then(()=>{
            ++nbBlockFetch;
            console.log(blockNumberArray.length,nbBlockFetch)
        })
